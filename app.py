...
# 保留 管理员(男)/(女)，只去掉没有实际区分的“管理员”汇总行，避免重复
...
active_slot_label = st.sidebar.radio("当前编辑", slot_labels, index=st.session_state.get("active_slot", 0), key="active_slot_radio")
...
selected_character = st.sidebar.selectbox("选择干员", character_names, key="selected_character")
...
with w_top1:
    selected_weapon_display = st.selectbox("选择武器", weapon_display_options, key="selected_weapon_display")
